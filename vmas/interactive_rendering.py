...
class InteractiveEnv:
    ...
    def _increment_selected_agent_index(self, index: int):#select index of agent
        index += 1
        if index == self.n_agents:
            index = 0
        return index

    def _cycle(self): #cycling states
    ...
    def _init_text(self):#initialize text
    ...
    def _write_values(self, index: int, message: str, font_size: int = 15):#save and write values
        self.env.unwrapped().viewer.text_lines[index].set_text(
            message, font_size=font_size
        )

    # keyboard event callbacks
    def _key_press(self, k, mod):#controling UP, DOWN, LEFT, RIGHT with W, S, A, D
    ...
    def _key_release(self, k, mod):#key changing
        from pyglet.window import key
...
def render_interactively(#gather all functions that interact with scenarios above
    scenario_name: str,
    control_two_agents: bool = False,
    display_info: bool = True,
    save_render: bool = False,
